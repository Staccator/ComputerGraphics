...
namespace ShapeDrafter
{
    public partial class MainWindow
    {
        ...
        private Color _defaultLineColor = Color.Green;
        public void ClearUnfinishedPolygon()
        {
            if (_started)
            {
                _newPolygon.ClearPolygon();
                _newPolygon = null;
                _started = false;
            }
        }
        ...
        private void DrawPolygon(Point point)
        {
            if (!_started)
            {
                ...
                _newPolygon = new Polygon(_defaultLineColor);
            ...
            if (length < 15)
            {
                if (_newPolygon.Vertices.Count >= 3)
                {
                    ...
                    DrawPoints(lastEdge.Points,_defaultLineColor);
                }
            ...
            DrawDot(point, Color.Crimson);
            DrawPoints(newEdge.Points,_defaultLineColor);
